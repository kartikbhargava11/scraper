...
base_llm_strategy = LLMExtractionStrategy(
	...
	instruction="Extract only real product records from the current product page content with 'name', 'short_description', 'price', 'brand', 'product code', 'availability' and 'specs' fields. For price, use the current selling price shown on the page, including the currency symbol when available. Do not use EMI, MRP, discount, shipping, tax, or related-product prices as the main price. If a price is visibly present in the content, do not leave it blank. Short_description should not exceed 100 words. If some information is genuinely missing, leave blank. Return valid JSON", # prompt
